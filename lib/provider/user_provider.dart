...
class UserNotifier extends StateNotifier<AppUser>{
  UserNotifier():super(AppUser(dep: 'dep', id: 'id', name: 'name', password: 'password', sex: 'sex'));
  ...
  Future signUpFunc({
    ...
    try{
      ...
      if(overlap){
        CustomDialog(context: context, title: '중복된 아이디가 존재합니다.\n다른 아이디를 사용해주세요.', buttonText: '확인', buttonCount: 1, func: (){
  ...
  Future LoginFunc({
    ...
    try{
      ...
      if(!loginState){
        CustomDialog(context: context, title: '로그인 오류\n로그인 정보를 확인해주세요.', buttonText: '확인', buttonCount: 1, func: (){
            Navigator.pop(context);
        });
      }
    }catch (e) {
  ...
  void LogoutFunc(){
    state = AppUser(dep: 'dep', id: 'id', name: 'name', password: 'password', sex: 'sex');
  }
  ...
  Future ChangeUserData(AppUser user, BuildContext context, WidgetRef ref) async {
    firestoreInstance.collection('user').where('id', isEqualTo: user.id).get().then((QuerySnapshot querySnapshot) {
            querySnapshot.docs.forEach((doc) {
              firestoreInstance.collection('user').doc(doc.id).update({
                ...
                CustomDialog(context: context, title: '변경이 완료되었습니다.\n재로그인 해주세요.', buttonText: '확인', buttonCount: 1, barrierDismissible: false, func: (){
